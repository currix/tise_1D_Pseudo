SUBROUTINE E1_ISQW(Iprint, I_toten)
  ...
  REAL(KIND = DP) :: error, E1_analytical, aj, ak
  REAL(KIND = DP), DIMENSION(:), ALLOCATABLE :: matrix_element, Total_E1
  REAL(KIND = DP), DIMENSION(:,:), ALLOCATABLE ::  matrix_x,  E1_numerical
  !
  ...
  !
  !
  ALLOCATE(E1_numerical(1:dim_BOX,1:I_toten), STAT = Ierr)
  IF (Ierr /= 0) THEN
     WRITE(*,*), "E1_numerical  allocation request denied."
     STOP
  ENDIF
  !
  !
  ...
  DO i_state = 1, I_toten
     ...
     WRITE(78,*) "#Aval_box(i)    E1_numerical**2"
     !
     DO i = 1, DIM_BOX
        !     
        matrix_element = Avec_box_x(:,i_state)*X_grid(:)*Avec_box_x(:,i)  
        !
        E1_numerical(i, i_state) = 0.0_DP
        Ifail = 0
        !
        CALL D01GAF(X_Grid, matrix_element, dim_X, E1_numerical(i, i_state), error, Ifail)
        !
        WRITE(*,15), i, "-th state energy: ", Aval_box(i), " <",i_state,"| X |Avec(",i,")> = ", E1_numerical(i, i_state)
        !
        Total_E1(i_state) = Total_E1(i_state) + (E1_numerical(i, i_state)**2)
        !
        ! SAVING E1
        WRITE(78,11)  Aval_box(i), E1_numerical(i, i_state)**2
        !
     ENDDO
     !
     CLOSE(UNIT = 78)
     ...
     !
     DO i = 1, DIM_BOX
        E1_analytical = DOT_PRODUCT(Avec_box(:,i_state),MATMUL(matrix_x,Avec_box(:,i)))
        !
        WRITE(*,15), i, "-th state energy: ", Aval_box(i), " <",i_state,"| X |Avec(",i,")> = ", E1_analytical
        !
     ENDDO
     !
     !
  ...
  WRITE(76,*) "#Aval_box(i)    E1_numerical"
  !
  DO i = 1, dim_BOX     !
     ! SAVING E1
     WRITE(76,12)  Aval_box(i), E1_numerical(i,1:I_toten)
     !
  ENDDO
  !
  CLOSE(UNIT = 76)
...
15 FORMAT (2X,I3,A,E16.8,2X,A,I3,A,I3,A,E17.8)
  ...
  IF (Ierr /= 0) THEN
     WRITE(*,*), "E1_numerical deallocation request denied."
     STOP
  ENDIF
  !
  !
