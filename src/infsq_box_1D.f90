PROGRAM SQBOX1Df90
  !
  !
  !     $Id: infsq_box_1D.f90,v 1.9 2014/02/02 10:28:31 curro Exp $
  !
  !
 ...
 ! REAL(KIND = DP), EXTERNAL :: POT
  ...
  !
  IF (Iprint>3) THEN 
     WRITE(*,*) Igs, IsaveEN, IsaveWF, IsaveBAS, I_sumr, I_toten, Iprint 
     WRITE(*,*)
  ENDIF
  !
  ...
  !
  IF (Iprint>3) WRITE(*,*) ' Reading reduced mass and IAD'
  !
  READ(UNIT=*,NML=INP_MASS)
  !
  IF (Iprint>3) THEN 
     WRITE(*,*) Iad, reduced_mass
     WRITE(*,*)
  ENDIF
  !
...
11 FORMAT (1X,E14.6,1X,300E17.8E3)
  ...
  IF(I_toten /= 0) THEN
     ! E1 CALCULATION 
     CALL E1_ISQW(Iprint, I_toten)
     !
     ! E2 CALCULATION 
     CALL E2_ISQW(Iprint, I_toten)
  ENDIF
  !
  !
  !
